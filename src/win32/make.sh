...
i686-pc-mingw32-gcc -o ospatrol-agent.exe -Wall  -DARGV0=\"ospatrol-agent\" -DCLIENT -DWIN32 -DOSPATROL icon.o os_regex/*.c os_net/*.c os_xml/*.c zlib-1.2.3/*.c config/*.c shared/*.c os_execd/*.c os_crypto/blowfish/*.c os_crypto/md5/*.c os_crypto/sha1/*.c os_crypto/md5_sha1/*.c os_crypto/shared/*.c rootcheck/*.c *.c -Iheaders/ -I./ -lwsock32
i686-pc-mingw32-gcc -o ospatrol-rootcheck.exe -Wall  -DARGV0=\"ospatrol-rootcheck\" -DCLIENT -DWIN32 icon.o os_regex/*.c os_net/*.c os_xml/*.c config/*.c shared/*.c win_service.c rootcheck/*.c -Iheaders/ -I./ -lwsock32
i686-pc-mingw32-gcc -o manage_agents.exe -Wall  -DARGV0=\"ospatrol-agent\" -DCLIENT -DWIN32 -DMA os_regex/*.c zlib-1.2.3/*.c os_zlib.c shared/*.c os_crypto/blowfish/*.c os_crypto/md5/*.c os_crypto/shared/*.c addagent/*.c -Iheaders/ -I./ -lwsock32
...
makensis ui.nsi
makensis ospatrol-installer.nsi
